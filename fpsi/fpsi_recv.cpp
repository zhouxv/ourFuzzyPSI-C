...
void FPSIRecv::init_inf() {
  ...
  // OKVS 初始化
  u64 okvs_size = PTS_NUM * BLK_CELLS * DIM * omega;

  rb_okvs.init(okvs_size, OKVS_EPSILON, OKVS_LAMBDA, OKVS_SEED);
  ...
  for (u64 i = 0; i < PTS_NUM * BLK_CELLS * DIM; i++) {
    for (u64 j = 0; j < omega; j++) {
      pre_ciphers.push_back(ct_zero_blocks[j]);
    }
  }
...
void FPSIRecv::msg_low_inf() {
  ...
  padding_keys(keys, rb_okvs.mN);

  spdlog::debug("keys {} values {}", keys.size(), pre_ciphers.size());
  ...
  timer.start();
  rb_okvs.encode(keys, pre_ciphers, PAILLIER_CIPHER_SIZE_IN_BLOCK, enconding);
  timer.end("recv_okvs_encode");
  insert_timer(timer);
  spdlog::info("recv okvs encoding 完成");

  coproto::sync_wait(sockets[0].flush());
  coproto::sync_wait(sockets[0].send(rb_okvs.mN));
  coproto::sync_wait(sockets[0].send(rb_okvs.mSize));
  ...
  u64 encoding_com_batch_size = rb_okvs.mSize / THREAD_NUM;
  ...
  auto encoding_com = [&](u64 thread_index) {
    ...
    u64 pt_count = (thread_index != THREAD_NUM - 1)
                       ? pts_batch_size
                       : PTS_NUM - pts_batch_size * (THREAD_NUM - 1);
...
void FPSIRecv::msg_low_lp() {
  ...
  auto post_process_lp_dec = [&](u64 thread_index) {
    ...
    for (u64 i = 0; i < res_size; i++) {
      ...
      //   cout << i % (12) << ": " << plainText.getElementVec(i)[0] << endl;
      // }
      plain_nums[i] = plainText.getElementVec(i)[0];
    }
  ...
  vector<u32> if_match_pt_nums(if_match_count, 0);
  for (u64 i = 0; i < if_match_count; i++) {
    if_match_pt_nums[i] = if_match_pt.getElementVec(i)[0];
  }
  ...
  for (auto num : if_match_pt_nums) {
    blake3_hasher_init(&hasher);
    blake3_hasher_update(&hasher, &num, sizeof(u32));
    blake3_hasher_finalize(&hasher, hash_out.data(), 16);
