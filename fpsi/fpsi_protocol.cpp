...
void run_high_dimension(const CLP &cmd) {
  ...
  ipcl::KeyPair if_match_key = ipcl::generateKeypair(2048, true);
  ipcl::KeyPair fm_key = ipcl::generateKeypair(2048, true);
  ipcl::terminateContext();
  ...
  // 接收方和发送方初始化
  FPSIRecv_H recv(DIM, DELTA, recv_size, METRIC, 1, recv_pts, psi_key.pub_key,
                  psi_key.priv_key, fm_key.pub_key, fm_key.priv_key,
                  if_match_key.pub_key, socketPair0);
  FPSISender_H sender(DIM, DELTA, send_size, METRIC, 1, send_pts,
                      psi_key.pub_key, fm_key.pub_key, if_match_key.pub_key,
                      if_match_key.priv_key, socketPair1);
  ...
  timer.start();
  // 使用 std::bind 将成员函数和对象绑定
  std::thread recv_msg(std::bind(&FPSIRecv_H::msg, &recv));
  std::thread send_msg(std::bind(&FPSISender_H::msg, &sender));
